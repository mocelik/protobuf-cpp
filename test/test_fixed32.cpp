#include "protobuf-cpp/Fixed32.h"
...
/**
 * Verify that encoding a Fixed32 only and always uses 4 bytes
 */
TEST(Fixed32, serialized_varint_encoded_size) {
    ASSERT_EQ(proto::Fixed32(0).serialize().size(), sizeof(std::uint32_t));
    static_assert(proto::Fixed32(0).serialize().size() == sizeof(std::uint32_t),
                  "Fixed32 serialized size should be 4 bytes");
}

TEST(Fixed32, serialize_deserialize) {
    auto test_values = {0x0000'0000u, 0x0000'00ffu, 0x0000'ff00u,
                        0x00ff'0000u, 0xff00'0000u, 0xffff'ffffu};

    for (auto v : test_values) {
        proto::Fixed32 fixed32_original(v);
        auto serialized = fixed32_original.serialize();
        auto deserialized = proto::Fixed32::deserialize(serialized);
